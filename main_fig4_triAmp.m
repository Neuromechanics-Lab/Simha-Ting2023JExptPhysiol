%% spindleMain_fig4_triangles with differnet conditioning amplitdues
...
if isempty(parms.date)
    ...
    parms.simNo=[4]; % just a sim number so as to not overwrite files
end
...
ampList = [0 0.25 0.5 0.75 1 1.25 1.5 1.75 2 3 7];
...
for i=1:numel(ampList)
    parms.condAmp = ampList(i)*MTUtoFibre; % in %L0; first, conditioning stretch amplitude
    parms.fibreType = 'Bag';
